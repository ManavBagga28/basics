...
import { CartEditRequestInput, CartRequestInput } from "~src/schema/cartRequest";
import { CartRepository } from "~src/repositories/cart.repository";
import { AuthorizeError, NotFoundError } from "~src/utils/error/errors";
import { logger } from "~src/utils/logger";
import { GetProductDetails, GetStockDetails } from "~src/utils/broker/api";
export const CreateCart = async (
  ...
  if (lineItem) {
    return repo.updateCartLineItem(lineItem.id, lineItem.qty + input.qty);
  }
...
export const GetCart = async (id: number, repo: CartRepository) => {
...
export const EditCart = async (
  ...
  const data = await repo.updateCartLineItem(input.id, input.qty);
  return data;
};
...
export const DeleteCart = async (
  ...
  await AuthorisedCart(input.id, input.customerId, repo);
  const data = await repo.deleteCartLineItem(input.id);
  return data;
};